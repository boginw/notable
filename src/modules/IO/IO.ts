...
export default class IO implements IIO {
	...
	public saveFile(path: string, contents: string): Promise<void> {
		let p = new Promise<void>((resolve: () => void, reject: (reason: string) => void) => {
			fs.writeFile(path, contents, (err: NodeJS.ErrnoException) => {
				let nErr: string = err ? err.message : "";
				this.callbackVoidResolve(nErr, resolve, reject);
			});
		});
		return p;
	}
	...
	public rename(filePath: string, newName: string): Promise<void> {
		return new Promise<void>((resolve: () => void, reject: (reason: string) => void) => {
			fs.rename(filePath, newName, (err?: NodeJS.ErrnoException) => {
				if (err) {
					reject(err.message);
					return;
				}
	...
	public exists(path: string): Promise<void> {
		return new Promise<void>((resolve: () => void, reject: (reason: string) => void) => {
			fs.access(path, fs.constants.R_OK | fs.constants.W_OK, (err: NodeJS.ErrnoException) => {
				if (err) {
					reject(err.message);
					return;
				}
	 ...
	 */
	public filePreview(pathToFile: string, length?: number): Promise<string> {
		// Overload methods are overrated
		let bufferLength = length || 100;
	 ...
	 */
	public fileStats(filePath: string): Promise<fs.Stats> {
		return new Promise<fs.Stats>(
			(resolve: (value: fs.Stats) => void, reject: (reason: string) => void) => {
	 ...
	 */
	private callbackVoidResolve(err: string, resolve: () => void, reject: (reason: string) => void): void {
		if (err) {
			reject(err);
			return;
		}
